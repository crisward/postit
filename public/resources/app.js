...
},{"./tags/app.tag":2,"./tags/board.tag":3,"./tags/note.tag":4,"riot":5}],2:[function(require,module,exports){
...
var riot = require('riot');
module.exports = 
riot.tag2('board', '<div class="alert" if="{!connected}"> <p>You\'ve been disconnected</p> </div> <button class="btn" onclick="{addnote}">Add Note</button> <note each="{note,idx in notes}" no-reorder riot-style="top:{note.top}px;left:{note.left}px;z-index:{idx==active?5:1}" idx="{idx}" editing="{note.editing}" text="{note.text}" chosen="{parent.noteSelected}" changed="{parent.noteChange}" remove="{parent.noteRemove}"></note>', 'board { border: 1px solid #ccc; background: #ccc; } .btn { border: 1px solid #ccc; background: rgba(255,255,255,0.8); border-radius: 4px; padding: 5px 10px; font-size: 14px; line-height: 18px; margin: 0 0 0 5px; display: inline-block; } .btn:active, .btn:focus { background: #fff; outline: 0px; } .alert { border: 1px solid #d00; background: #d88; color: #fff; margin: 0 0 5px 0; } .alert p { margin: 10px; }', '', function(opts) {
this.notes = [];
  ...
  return function(timeout) {
    ...
    if (!_this.connected) {
      ...
      return setTimeout(function() {
        ...
        if (!_this.connected) {
          return _this.reconnect(timeout);
        }
      }, timeout);
    }
  };
...
this.noteRemove = (function(_this) {
  return function(idx) {
    console.log('removing', idx, _this.notes[idx]);
    if (!_this.notes[idx].editing) {
      _this.socket.send(JSON.stringify({
        type: "notes:remove",
        note: _this.notes[idx]
      }));
    }
    return _this.update();
  };
